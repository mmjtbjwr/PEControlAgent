...
from OptimizationTool import find_optimization_algorithm_for_boost
...
def manager_response(self, messages, sender, config):
    if state_manager.state == "initial":
        ...
        return True, "Conversation complete. The Boost converter controller design process is finished."
    else:
...
manager_agent.register_reply(
    lambda message: state_manager.state in ["initial", "model_configured", "objectives_defined", "controller_designed", "optimization_designed"],
    manager_response
)
...
optimization_algorithm_design_agent.register_reply(
    lambda message: state_manager.state == "optimization_design",
...
# Create GroupChat
groupchat = GroupChat(
    agents=[human, manager_agent, model_design_agent, objective_agent, controller_algorithm_design_agent, optimization_algorithm_design_agent],
    messages=[],
    max_round=10
)
...
if __name__ == "__main__":
    ...
    10. ManagerAgent: Conclude the conversation
