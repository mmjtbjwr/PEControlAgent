...
def find_model(requirements):
    model_info = model_config_tool.find_or_create_model(requirements)
    if model_info:
        state_manager.model_info = model_info
        return f"Model configured and modified successfully. Model ID: {model_info['model_id']}, FMU Address: {model_info['address']}"
    else:
        return "Unable to configure or create a model meeting the requirements."
...
model_design_agent = ConversableAgent(
    ...
    system_message=f"""You are an agent responsible for configuring and modifying appropriate models for controller design. 
    When asked to find a suitable model, always use the following function to configure and modify the model:
    {find_model.__name__}
    Return the exact output of this function without any modification.
...
def controller_algorithm_design_response(self, messages, sender, config):
    if state_manager.model_info is None:
        return True, "Error: Model information is not available."
    requirements = {
