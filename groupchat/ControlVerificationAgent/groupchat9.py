...
class StateManager:
    def __init__(self):
        ...
        self.fmu_path = None
        self.control_algorithm_name = None
        self.control_algorithm_params = None
        self.optimization_algorithm_name = None
        self.optimization_algorithm_params = None
        self.simulation_params = None
        self.evaluate_params = None
...
def manager_response(self, messages, sender, config):
    if state_manager.state == "initial":
        ...
        return False, "ControlVerificationAgent, please begin the verification process for the Boost converter controller design."
        ...
        1. Mean Absolute Error (MAE): {verification_result.mae:.4f}
           Target: Below 2
           Status: {'Passed' if verification_result.mae < 2 else 'Failed'}

        2. Overshoot: {verification_result.overshoot:.2f}%
           Target: Below 8%
           Status: {'Passed' if verification_result.overshoot < 8 else 'Failed'}

        3. Settling Time: {verification_result.settling_time:.4f} seconds
           Target: Below 0.5 seconds
           Status: {'Passed' if verification_result.settling_time < 0.5 else 'Failed'}

        4. Optimized Controller Parameters:
           Voltage PI: Kp = {verification_result.best_params['voltage_k']:.4f}, Ti = {verification_result.best_params['voltage_Ti']:.4f}
           Current PI: Kp = {verification_result.best_params['current_k']:.4f}, Ti = {verification_result.best_params['current_Ti']:.4f}

        Overall Assessment:
        {'All requirements are met. The controller design is successful.' if all([verification_result.mae < 2, verification_result.overshoot < 8, verification_result.settling_time < 0.5]) else 'Some requirements are not met. Further optimization or redesign may be necessary.'}

        The Boost converter controller design process has been completed.

        TERMINATE
        """
        state_manager.state = "analysis_completed"
...
def find_model(requirements):
    ...
    if model_info:
        ...
        state_manager.fmu_path = model_info['address']
...
def controller_algorithm_design_response(self, messages, sender, config):
    ...
    state_manager.control_algorithm_name = "duallooppid"
    state_manager.control_algorithm_params = {
...
def optimization_algorithm_design_response(self, messages, sender, config):
    ...
    state_manager.optimization_algorithm_name = "pso"
    state_manager.optimization_algorithm_params = {"swarm_size": 5, "max_iterations": 5, "w": 0.5, "c1": 1.5, "c2": 1.5}
...
def verification_response(self, messages, sender, config):
    verification_input = VerificationInput(
        fmu_path=state_manager.fmu_path,
        control_algorithm_name=state_manager.control_algorithm_name,
        control_algorithm_params=state_manager.control_algorithm_params,
        optimization_algorithm_name=state_manager.optimization_algorithm_name,
        optimization_algorithm_params=state_manager.optimization_algorithm_params,
        simulation_params={
            "simulation_time": 1,
            "target_voltage": 160,
            "initial_voltage": 80,
    ...
    return True, f"Verification completed. Result: {verification_result.dict()}"

control_verification_agent = ConversableAgent(
    name="ControlVerificationAgent",
    system_message="You are responsible for verifying the Boost converter controller design. When instructed to begin verification, use the predefined verification input to start the process.",
    llm_config=llm_config
)
...
# 创建自定义的 GroupChatManager
class CustomGroupChatManager(GroupChatManager):
    def _process_received_message(self, message: str, sender: ConversableAgent, silent: bool) -> str:
        if message.strip().endswith("TERMINATE"):
            print(message.strip()[:-9])  # 打印分析结果，但不包括 TERMINATE
            print("\nConversation ended successfully.")
            self._terminate_chat = True
            return ""
        return super()._process_received_message(message, sender, silent)

# 使用自定义的 GroupChatManager
manager_chat = CustomGroupChatManager(groupchat=groupchat, llm_config=llm_config)
...
if __name__ == "__main__":
    ...
    10. ManagerAgent: Instruct ControlVerificationAgent to begin verification
