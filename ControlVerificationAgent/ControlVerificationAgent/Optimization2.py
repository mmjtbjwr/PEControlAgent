...
class ControllerFactory:
    @staticmethod
    def create_controller(controller_name: str, **params):
        if controller_name.lower() == "pi":
            return PIController(**params)
...
class OptimizationTool(ABC):
    def __init__(self, control_algorithm: str, model_path: str, population_size: int,
    ...
    def simulate(self, fmu_path: str, simulation_time: float, time_step: float,
                 controller_name: str, control_params: ControlParams):
        ...
        voltage_controller = ControllerFactory.create_controller(controller_name, **control_params.voltage_control)
        current_controller = ControllerFactory.create_controller(controller_name, **control_params.current_control)
...
class PSOOptimizeTool(OptimizationTool):
    def optimize(self) -> OptimizationResult:
        ...
        for i in range(self.num_variables):
            particles[:, i] = particles[:, i] * (self.variable_ranges[i][1] - self.variable_ranges[i][0]) + \
                              self.variable_ranges[i][0]
        ...
        for iteration in range(self.max_iterations):
            ...
            print(
                f"Iteration {iteration + 1}: Best position = {global_best_position}")
...
if __name__ == "__main__":
    model_path = r"H:\project_code\pso-pid\boosttwopid\boost_converternopid.fmu"
    population_size = 10
    ...
    simulation_time = 1.0
    time_step = 0.0001
    control_algorithm = "PI"  # Now we pass the name of the control algorithm

    pso_tool = PSOOptimizeTool(control_algorithm, model_path, population_size, num_variables, variable_ranges, max_iterations, simulation_time, time_step)
    pso_result = pso_tool.optimize()
    print("PSO Best Result:", pso_result)

    # ga_tool = GAOptimizeTool(control_algorithm, model_path, population_size, num_variables, variable_ranges, max_iterations, simulation_time, time_step)
